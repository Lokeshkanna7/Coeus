...
import {
  ...
  Send, FileText, UserCheck, MessageSquare, CheckCircle,
  Target, ClipboardList, Rocket, LifeBuoy, ArrowUp
} from 'lucide-react';
import './App.css';
...
<Link to="/" className="flex items-center interactive group">
  {/* Modified div: removed gradient and hover effects */}
  <div className="w-30 h-30 rounded-lg flex items-center justify-center mr-3 p-2">
    <img src="/logo/Screenshot 2025-09-08 123829.png" alt="COEUS Logo" className="w-full h-full object-contain" />
  </div>
</Link>
...
// Loading Screen Component with smaller video
const LoadingScreen = ({ isLoading, progress, displayText }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 flex items-center justify-center z-50 overflow-hidden">
      ...
      <div className="text-center relative w-full max-w-md mx-4 z-10"> {/* Reduced from max-w-2xl to max-w-md */}
        
        {/* Smaller Video Container */}
        <div className="mb-4 relative"> {/* Reduced margin from mb-8 to mb-4 */}
          <div className="relative mx-auto w-full max-w-xs"> {/* Reduced from max-w-md to max-w-xs */}
            
            {/* Smaller glow effect */}
            <div className="absolute -inset-2 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 rounded-xl blur-lg animate-pulse" /> {/* Reduced inset */}
            
            {/* Smaller main container */}
            <div className="relative bg-gradient-to-br from-cyan-900/40 via-blue-900/40 to-purple-900/40 rounded-lg p-0.5 backdrop-blur-xl border border-cyan-500/30 shadow-md"> {/* Reduced padding, rounded, shadow */}
              
              <div className="relative rounded-md overflow-hidden bg-black/20 min-h-[120px] flex items-center justify-center"> {/* Reduced min-height from 200px to 120px */}
                <video 
                  autoPlay 
                  loop 
                  muted 
                  playsInline
                  className="w-full h-full object-cover rounded-md" /* Reduced rounded-lg to rounded-md */
                  onError={(e) => {
                ...
                <div 
                  id="video-fallback"
                  className="absolute inset-0 hidden items-center justify-center flex-col bg-gradient-to-br from-cyan-900/50 to-blue-900/50 rounded-md" /* Reduced rounded */
                >
                  <div className="text-center">
                    <div className="text-cyan-400 text-xl font-bold mb-1 animate-pulse">COEUS</div> {/* Reduced from text-3xl */}
                    <div className="text-cyan-300 text-lg font-medium mb-2">LABS</div> {/* Reduced from text-xl */}
                    <div className="text-cyan-200 text-xs">PRECISION AND INTELLIGENCE</div> {/* Reduced from text-sm */}
                  </div>
                </div>

                {/* Smaller loading overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent flex items-end justify-center pb-2"> {/* Reduced padding */}
        ...
        {/* Progress section remains the same or can also be reduced */}
        <div className="mb-6"> {/* Reduced margin */}
          {/* Smaller progress ring */}
          <div className="relative mx-auto w-20 h-20 mb-4"> {/* Reduced size */}
            {/* Outer glow */}
            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-cyan-500/20 to-blue-500/20 animate-ping" style={{ animationDuration: '3s' }} />
            ...
            {/* Inner content */}
            <div className="absolute inset-3 rounded-full bg-gradient-to-br from-cyan-900/40 to-blue-900/40 backdrop-blur-sm border border-cyan-500/20 flex items-center justify-center"> {/* Reduced inset */}
              <div className="text-center">
                <span className="text-cyan-300 text-xl font-bold block">{Math.round(progress)}%</span> {/* Reduced text size */}
                <span className="text-cyan-400 text-xs font-mono">COMPLETE</span> {/* Reduced text size */}
              </div>
            </div>
          </div>
          ...
          <div className="mb-4"> {/* Reduced margin */}
            <p className="text-cyan-300 mb-3 font-mono text-lg tracking-wider"> {/* Reduced text size */}
              <span className="inline-block min-w-[180px] text-center"> {/* Reduced min-width */}
                {displayText}
            ...
            <div className="w-48 mx-auto bg-cyan-900/30 rounded-full h-1.5 mb-2"> {/* Reduced width and height */}
              <div 
          ...
          <div className="flex justify-center space-x-4 mb-4"> {/* Reduced spacing and margin */}
            {[<Cpu key="cpu" />, <Satellite key="sat" />, <Brain key="brain" />, <Cloud key="cloud" />].map((Icon, index) => (
              <div
                key={index}
                className="w-8 h-8 bg-cyan-900/30 rounded-lg flex items-center justify-center border border-cyan-500/20 animate-bounce" /* Reduced size */
                style={{ animationDelay: index * 0.3 + 's' }}
              >
                {React.cloneElement(Icon, { 
                  className: "text-cyan-400 w-4 h-4" // Reduced icon size
                })}
              </div>
            ))}
          </div>

          {/* Smaller status message */}
          <div className="text-cyan-400 font-mono text-xs tracking-widest"> {/* Reduced text size */}
            {progress < 50 ? "SYSTEM BOOTING" : progress < 80 ? "CALIBRATING" : "FINALIZING"}
          </div>
        </div>
      </div>
    </div>
...
const DroneCompanyWebsite = () => {
  ...
  // Enhanced loading logic with minimum display time
  useEffect(() => {
    const hasVisited = localStorage.getItem('coeus_visited');
    const minimumLoadingTime = 4000; // 4 seconds minimum
    const startTime = Date.now();

    // Always show loading screen for at least 4 seconds for better UX
    ...
    const loadingInterval = setInterval(() => {
      setProgress(prev => {
        ...
        // Ensure we show loading for minimum time even if progress completes
        const elapsedTime = Date.now() - startTime;
        if (newProgress >= 100 && elapsedTime >= minimumLoadingTime) {
          clearInterval(loadingInterval);
          setTimeout(() => {
            setIsLoading(false);
            localStorage.setItem('coeus_visited', 'true');
          }, 500);
          return 100;
        }
        
        // Cap at 100% but keep loading until minimum time
    ...
    const textInterval = setInterval(() => {
      ...
      const currentTextUpdate = textUpdates.find(t => currentProgress >= t.percent && displayText !== t.text);
      if (currentTextUpdate) {
        setDisplayText(currentTextUpdate.text);
      }
    }, 500);
      ...
      {[
        {
          ...
          imageSrc: "/images/Automated Inspections.png" // Placeholder for the image
        },
        {
          ...
          imageSrc: "/images/Predictive Maintenance.png" // Placeholder for the image
        },
        {
          title: "Real-time Analytics",
          description: "Instant insights into grid health and performance metrics",
          icon: <BarChart size={48} className="text-cyan-400" />,
          imageSrc: "/images/Real-time Analytics.png" // Placeholder for the image
        }
      ].map((solution, index) => (
        <div key={index} className="bg-gray-800/50 backdrop-blur-md rounded-2xl p-8 border border-cyan-500/20 interactive feature-card opacity-0 transform translate-y-10 transition-all duration-700 hover:border-cyan-500/40 hover:bg-gray-800/70">
          ...
          {solution.imageSrc && (
            <img 
              ...
              alt={solution.title} 
              className="w-full h-48 object-cover rounded-lg mb-6 shadow-lg" // Added styling for the image
            />
          )}
    ...
    <div className="flex flex-col space-y-12 max-w-4xl mx-auto">
      {[
        ...
        },
        {
          ...
          image: "/images/edge_detection.png"
        },
        {
          step: "4",
          ...
          image: "/images/sap_integeration.png"
        },
        {
          step: "5",
          ...
          image: "/images/field_response.png"
        }
      ].map((item, index) => (
      ...
      {[
        {
          ...
          image: "/images/Prevent Outages Before They Happen.png"
        },
        {
          ...
          image: "/images/Cut Operational Costs.png"
        },
        {
          ...
          // IMPORTANT: Check if this filename is correct. It might have a typo.
          image: "/images/Scale with Intelligence.png" 
        },
        {
          ...
          icon: <Activity size={32} className="text-blue-400" />,
          image: "/images/Sustainability & Safety.png"
        }
      ].map((item, index) => (
        ...
        {/* Contact Section with Direct Map Links */}
<section id="contact" className="py-20 relative z-10">
    <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
            ...
            {/* MODIFIED: Removed all form fields and vertically centered the content */}
            <div className="bg-gray-800/50 backdrop-blur-md rounded-2xl p-6 border border-cyan-500/20 shadow-sm flex flex-col justify-center items-center">
            ...
            <div className="space-y-8">
                <div className="bg-gray-800/50 backdrop-blur-md rounded-2xl p-6 border border-cyan-500/20 shadow-sm">
                    <h3 className="text-xl font-bold mb-6 text-cyan-400">Our Locations</h3>
                    
                    <div className="flex flex-col space-y-4">
                        <a
                            href="https://maps.app.goo.gl/gAJ616AmgR1h9nw1A"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-semibold py-3 px-4 rounded-lg text-center transition-all shadow-md"
                        >
                            Florida, USA
                        </a>

                        <a
                            href="https://maps.app.goo.gl/aPfaADLSG3kgg4W7A"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-semibold py-3 px-4 rounded-lg text-center transition-all shadow-md"
                        >
                        ...
                        <a
                            href="https://maps.app.goo.gl/xvrHqkgwCCnEnjmBA"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-semibold py-3 px-4 rounded-lg text-center transition-all shadow-md"
                        >
                        ...
                        <a
                            href="https://maps.app.goo.gl/h8CqUENXfReu5kRz8"
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-semibold py-3 px-4 rounded-lg text-center transition-all shadow-md"
                        >
        ...
        <footer className="border-t border-cyan-500/20 py-16 relative z-10">
            <div className="container mx-auto px-4">
                ...
                <p className="text-cyan-300">© 2025 COEUS. All rights reserved.</p>
...
// Careers Page Component (from previous step)
const CareersPage = () => {
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
    const [filter, setFilter] = useState('All');
    
    const formLink = "https://forms.gle/your-form-link";
    
    const jobListings = [
        { title: "Senior Drone Engineer", department: "Engineering", location: "Kolkata, India", type: "Full-time", experience: "5+ years", skills: ["Drone Technology", "IoT", "Python", "CAD"] },
        { title: "AI/ML Specialist", department: "Research & Development", location: "Remote", type: "Full-time", experience: "4+ years", skills: ["Machine Learning", "Python", "TensorFlow", "Data Analysis"] },
        { title: "IoT Solutions Architect", department: "Engineering", location: "Kolkata, India", type: "Full-time", experience: "6+ years", skills: ["IoT Architecture", "Cloud Computing", "Embedded Systems", "Security"] },
        { title: "SAP Integration Developer", department: "IT", location: "Hybrid", type: "Contract", experience: "5+ years", skills: ["SAP CPI", "BTP", "API Integration", "Java"] },
    ];
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="text-center mb-20">
              ...
              <p className="text-xl text-cyan-200 max-w-3xl mx-auto">
                Join a revolutionary team that's transforming electrical infrastructure
            ...
            {/* Why Join Us Section */}
            <div className="mb-20">
              <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 text-white">Why Join COEUS?</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {[
                  { icon: <Sparkles size={32} />, title: "Innovate & Impact", description: "Work on challenging projects that have a real-world impact on grid reliability and safety." },
                  { icon: <Award size={32} />, title: "Growth & Learning", description: "We invest in your professional development with continuous learning opportunities." },
                  { icon: <Users size={32} />, title: "Collaborative Culture", description: "Be part of a diverse, inclusive, and supportive team that values every voice." },
                  { icon: <Heart size={32} />, title: "Great Benefits", description: "Enjoy competitive salaries, comprehensive health benefits, and a flexible work environment." },
                ].map((item, index) => (
                  <div key={index} className="bg-gray-800/50 backdrop-blur-md rounded-2xl p-8 border border-cyan-500/20 text-center hover:border-cyan-400/50 hover:-translate-y-2 transition-all duration-300">
            ...
            <div className="bg-gray-800/50 backdrop-blur-md rounded-2xl p-8 md:p-12 border border-cyan-500/20 shadow-lg mb-20">
                <h2 className="text-3xl md:text-4xl font-bold mb-4 text-center text-cyan-400">Current Openings</h2>
                <p className="text-center text-cyan-200 mb-8">Find your next opportunity in our list of open positions.</p>
                ...
                {filteredJobs.length > 0 ? filteredJobs.map((job, index) => (
                    <div key={index} className="bg-gray-900/40 rounded-xl border border-cyan-500/20 overflow-hidden transition-all duration-300 hover:border-cyan-400/50 hover:shadow-2xl hover:shadow-cyan-500/10 group">
                        <div className="p-6 md:p-8">
                            ...
                            <div className="flex flex-wrap gap-x-6 gap-y-3 text-cyan-300 mb-6 border-b border-cyan-500/10 pb-4">
                                ...
                                <span className="flex items-center"><Clock size={16} className="mr-2" /> {job.experience}</span>
                            </div>
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                                <div className="mb-4 md:mb-0">
                                    ...
                                    {job.skills.map((skill, skillIndex) => (
                                        <span key={skillIndex} className="bg-cyan-900/40 text-cyan-200 px-3 py-1 rounded-full text-sm font-medium">{skill}</span>
                                    ))}
                                    </div>
                                </div>
                                <button onClick={() => window.open(formLink, "_blank")} className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-3 px-8 rounded-lg hover:from-cyan-400 hover:to-blue-500 transition-all transform hover:scale-105 shadow-lg shadow-cyan-500/20 interactive flex items-center gap-2">
                    ...
                    <div className="text-center py-10">
                        <p className="text-cyan-200 text-lg">No open positions in this department at the moment. Please check back later!</p>
                    </div>
                )}
                </div>
            </div>
            ...
            <div>
              <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 text-white">Our Hiring Process</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 text-center">
                {[
                  { icon: <FileText />, title: "1. Application", description: "Submit your resume through our online portal. We review every application carefully." },
                  { icon: <UserCheck />, title: "2. Initial Screening", description: "Our HR team will reach out for a preliminary chat to get to know you better." },
                  { icon: <MessageSquare />, title: "3. Technical Interview", description: "Meet with the team to discuss your skills, experience, and solve technical challenges." },
                  { icon: <Award />, title: "4. Final Offer", description: "Congratulations! If you're a match, we'll extend an offer to welcome you to the team." },
                ].map((step, index) => (
                  <div key={index} className="relative">
                    <div className="bg-gray-800/50 backdrop-blur-md rounded-2xl p-8 border border-cyan-500/20 h-full">
                      ...
                      <h3 className="text-xl font-bold text-white mb-3">{step.title.substring(3)}</h3>
                      <p className="text-cyan-200">{step.description}</p>
                    </div>
                    {/* Connector Line for Desktop */}
                    {index < 3 && <div className="hidden lg:block absolute top-1/2 -right-4 w-8 h-px bg-cyan-500/30"></div>}
                  </div>
                ))}
              </div>
            </div>
...
// FINAL CORRECTED TALENT SOLUTIONS PAGE
const TalentSolutionsPage = () => {
  ...
  return (
    <div className="min-h-screen text-gray-100 font-sans w-full overflow-x-hidden relative">
      ...
      <div className="w-full max-w-full overflow-hidden px-4 py-12 pt-20 relative z-10">
        {/* FIX: Removed `space-y-28` from this container. Spacing will now be handled by each section individually. */}
        <div className="mx-auto max-w-7xl">
          
          {/* Hero Section */}
          <section className="text-center">
            <div className="inline-block bg-cyan-500/10 px-4 py-2 rounded-full mb-6 border border-cyan-500/30">
                <span className="text-cyan-300 text-sm font-medium">Empowering Your Workforce</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500">
              Advanced Talent Solutions
            </h1>
            <p className="text-xl text-cyan-200 max-w-3xl mx-auto">
              Bridge the skills gap in emerging technologies with our specialized training, staffing, and consultation for advanced drone and AI monitoring systems.
            </p>
          </section>
  
          {/* Our Core Offerings Section */}
          {/* FIX: Added `mt-28` for explicit top margin. */}
          <section className="mt-28">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {[
                      { title: "Corporate Training Programs", description: "Comprehensive training on drone operation, data analysis, and system maintenance.", icon: <Calendar />, features: ["Certified Trainers", "Hands-on Workshops", "Custom Curriculum", "Ongoing Support"] },
                      { title: "Strategic Consultation", description: "Expert guidance on implementing drone monitoring solutions in your organization.", icon: <Users />, features: ["Strategy Development", "Technology Assessment", "Implementation Planning", "ROI Analysis"] },
                      { title: "Specialized Staffing", description: "Access to skilled professionals for project-based work or temporary needs.", icon: <Briefcase />, features: ["Quick Deployment", "Vetted Professionals", "Flexible Terms", "Quality Assurance"] },
                      { title: "Custom Solution Development", description: "Tailored programs and integrations to meet your organization's specific requirements.", icon: <Cog />, features: ["Needs Assessment", "Custom Development", "Integration Support", "Continuous Maintenance"] }
                  ].map((service, index) => (
                      <div key={index} className="feature-card opacity-0 transform translate-y-10 transition-all duration-700 bg-gray-800/50 backdrop-blur-md rounded-2xl p-8 border border-cyan-500/20 shadow-lg group hover:border-cyan-400/80 hover:-translate-y-2 interactive" style={{ transitionDelay: `${index * 150}ms` }}>
                              ...
                              <div>
                                  ...
                                  <ul className="space-y-2">
                                      {service.features.map((feature, fIndex) => (
                                          <li key={fIndex} className="flex items-center text-cyan-300">
                                              <CheckCircle size={16} className="text-cyan-500 mr-3 flex-shrink-0" />
                                              <span>{feature}</span>
                                          </li>
                                      ))}
                                  </ul>
          ...
          {/* Our Partnership Process Section */}
          {/* FIX: Added `mt-28` for explicit top margin. */}
          <section className="text-center relative mt-28">
              <h2 className="text-3xl md:text-4xl font-bold text-center mb-16">
              ...
              <div className="relative grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  ...
                  <div className="hidden lg:block absolute top-1/2 left-0 w-full h-px -mt-8">
                      <svg width="100%" height="2" className="overflow-visible">
                         <line x1="0" y1="1" x2="100%" y2="1" strokeWidth="2" strokeDasharray="8 8" className="stroke-cyan-500/30" />
                      </svg>
                  </div>
                  {[
                      { icon: <Target />, title: "Discovery & Assessment", description: "We start by understanding your unique challenges, goals, and existing infrastructure." },
                      { icon: <ClipboardList />, title: "Customized Planning", description: "A tailored solution is designed, outlining training modules, staffing needs, or a tech roadmap." },
                      { icon: <Rocket />, title: "Implementation & Deployment", description: "We execute the plan, whether it's delivering training, placing talent, or building solutions." },
                      { icon: <LifeBuoy />, title: "Support & Evolution", description: "Our partnership continues with ongoing support to ensure long-term success and adaptation." }
                  ].map((step, index) => (
                      <div key={index} className="feature-card opacity-0 transform translate-y-10 transition-all duration-700 relative z-10" style={{ transitionDelay: `${index * 150}ms` }}>
                          <div className="bg-gray-800/80 backdrop-blur-lg rounded-2xl p-8 border border-cyan-500/20 h-full flex flex-col items-center">
                              ...
                              <p className="text-cyan-300 flex-grow">{step.description}</p>
                          </div>
                      </div>
                  ))}
              </div>
          ...
          <section className="mt-28">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {[
                      "Industry-leading expertise in drone & AI", "Proven track record with enterprise clients", "Flexible engagement models",
                      "Access to a pre-vetted talent pool", "Accelerated time-to-competency", "Measurable ROI on talent investment"
                  ].map((benefit, index) => (
                      <div key={index} className="feature-card opacity-0 transform translate-y-10 transition-all duration-700 bg-gray-800/50 backdrop-blur-md rounded-2xl p-6 border border-cyan-500/20 flex items-center gap-4" style={{ transitionDelay: `${index * 100}ms` }}>
                         <div className="flex-shrink-0 w-12 h-12 bg-cyan-900/50 rounded-lg flex items-center justify-center text-cyan-400">
                             <CheckCircle size={24} />
                         </div>
                         <p className="flex-1 text-lg font-medium text-cyan-100">{benefit}</p>
                      </div>
                  ))}
              </div>
          </section>
          ...
          {/* CTA Section */}
          {/* FIX: Added `mt-28` for explicit top margin. */}
          <section className="mt-28 feature-card opacity-0 transform translate-y-10 transition-all duration-700 bg-gradient-to-br from-cyan-600/80 to-blue-700/80 rounded-2xl p-12 text-white text-center border border-cyan-400/50 shadow-2xl shadow-cyan-500/10">
              <h3 className="text-4xl font-bold mb-4">Transform Your Team's Capabilities</h3>
              <p className="text-xl mb-8 max-w-3xl mx-auto opacity-90">
                Ready to empower your organization with the skills for tomorrow? Let's discuss how our talent solutions can drive your success.
              </p>
              <button className="bg-white text-cyan-700 font-bold py-4 px-10 rounded-xl hover:bg-gray-200 transition-all transform hover:scale-105 shadow-lg interactive flex items-center gap-3 mx-auto">
                  <Phone size={20} />
